...
struct META_BUFFER {
	 ...
	 *   11 (use BT only): data packets for BT, high priority.
	 *   12 (use BT only): data packets for BT, normal priority.
	 *   13 (use TwoWay naive)
	 *   14 (use TwoWay naive)
...
*/
    ...
    int CanLargePathTransmit(int saveBytes, SUBFLOW_MSG * msg, int schedDecision, int otherSubflowNo, int direction);
	...
	int TransmitFromMetaBuffer(SUBFLOW_MSG * msg, int bReinject);
