...
*/
	...
	if (/*space <= 0 || */tp->write_seq - tp->snd_nxt > space) {
		//ReportInfo("[avail-space temp una] 5 write-nxt=%u snd=%u cwnd=%u inflight=%u space=%u delta=%d", tp->write_seq - tp->snd_nxt, tp->snd_nxt-tp->snd_una, tp->snd_cwnd * tp->mss_cache, in_flight*tp->mss_cache, space, in_flight * tp->mss_cache - (tp->snd_nxt-tp->snd_una));
		return 0;
	}
