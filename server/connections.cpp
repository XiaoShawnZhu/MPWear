...
DWORD subflowLastAck1 = 0, subflowLastAck2 = 0;
DWORD subflowDupACK1 = 0, subflowDupACK2 = 0;
...
int CONNECTIONS::Setup() {
	...
	if (proxyMode == PROXY_MODE_LOCAL) {
		...
		if (bind(localListenFD, (struct sockaddr *)&serverAddr, sizeof(serverAddr))) return R_FAIL;
		if (listen(localListenFD, 32)) return R_FAIL;

		peers[0].fd = localListenFD;
		peers[0].events = POLLRDNORM;
		nextConnID = 1;	//a valid connID is always positive

		InfoMessage("Local proxy on. Listening port %d", LOCAL_PROXY_PORT);
	...
	for (int i = subflows.n + 1; i < MAX_FDS; i++) {
		if (peers[i].fd < 0) {
			...
			if (proxyMode == PROXY_MODE_LOCAL) {
				...
				if (proxyMode == PROXY_MODE_LOCAL && ci.serverIP == rpIPAddress) {
				...
				peersExt[i].bSentSYNToSubflow = 0;
				...
				InfoMessage("New incoming connection: %s", DumpConnection(nextConnID));	
			...
			// Remote proxy mode
				...
				//if (PROXY_SETTINGS::bLateBindingRP) KERNEL_INTERFACE::NewConnection(connID);

				InfoMessage("[%llu] New outgoing connection: %s", get_current_microsecond(), DumpConnection(connID));
				ci.hints.print();
				// use OWD mapping, no need to reset 
				//kernelInfo.ResetOWD();
			}
...
int BUFFER_SUBFLOWS::UpdateSubflowACKStatus(SUBFLOW_MSG * msg, int subflowNo) {
	...
	//InfoMessage("MSG SEQ: %llu, ACK: %llu", seq, ack);
	if (ack >= seq) {
		msg->bSubflowAcked = 1;
		return 1;
	}
	if (ack + 4294967296 - seq < 8000000) {
		msg->bSubflowAcked = 1;
		return 1;
	}
	return 0;
...
int CONNECTIONS::TransferFromTCPToMetaBuffer(int fromPollPos, int fromFD, uint64_t ts) {
	...
	// Scheduling decision
	//int toPollPos = (subflows.*subflows.SelectSubflowFunc_Main)(connID);
...
*/
		...
		while (1) {
			...
			if (n>0) {
				...
				if (PROXY_SETTINGS::bDumpIO) {
					DumpIO(TCPSOCKET_2_SUBFLOWBUFFER, n, toPollPos, connID, seq);
				}
				*/
				if (bFirst) {
	...
	//InfoMessage("------ Exit loop %d", bRemoved);
	...
	if (pMsgTail->msgType == SUBFLOW_MSG::MSG_CLOSE && pMsgTail->closeReason == SUBFLOW_MSG::MSG_CLOSE_FIN) {
		if (tickCount - lastSubflowActivityTick >= delayedFins.d1) {
			//delay it
	...
	//InfoMessage("Add to meta buffer: %d", metaBuffer.GetSize(1));

	// connection stats

	//conns.connTab[connID].bytesInSubflowBuffer += pMsgTail->bytesOnWire;
	...
	*/
...
#ifdef REINJECTION
	SUBFLOW_MSG * newMsg = pMsgTail;
	// Copy message to Unacked queue
	SUBFLOW_MSG * pCurrMsg = NULL;
	if (newMsg->msgType == SUBFLOW_MSG::MSG_DATA) {
		newMsg->subflowSeq = metaSeq;
		metaSeq++;
		pCurrMsg = CopyFromMsgBufToUna(toPollPos, newMsg);
#ifdef DEBUG_META_SEQ
		PrintUnaQueueSimple(toPollPos);
#endif
#ifdef DEBUG_UNA
		//InfoMessage("Add message to subflow %d:", toPollPos);
	...
	//if (subflowOutput.TransferFromTCPToSubflows(toPollPos, peers[toPollPos].fd, 0) > 0) {
...
#ifdef REINJECTION
	if (pCurrMsg)
		GetTCPSeqForUna(toPollPos, pCurrMsg);
#endif
...
int CONNECTIONS::TransferFromSubflowsToTCP(int fromPollPos, int fromFD) {
	...
	while (1) {
		if (header[8] == 8) {
			...
			int n = 0;
			switch (subflowType) {
//#if TRANS == 3		//read from subflow
			//case SUBFLOWS::SCTP_SUBFLOW:
			//	n = sctp_recvmsg(fromFD, pMsg->pMsgData + pMsg->bytesOnWire - 8 - pMsg->bytesLeft, pMsg->bytesLeft,
			//	NULL, NULL, NULL, NULL);
			//	break;
//#elif TRANS == 1
...
//#elif TRANS == 2
			case SUBFLOWS::UDP_SUBFLOW:
				n = udpRead(fromFD, pMsg->pMsgData[0] + pMsg->bytesOnWire - 8 - pMsg->bytesLeft[0], pMsg->bytesLeft[0]);
				break;
//#endif
			default:
				break;
			...
			if (n > 0) {
				...
				if (FindRequest(rr, pMsg->pMsgData + pMsg->bytesOnWire - 8 - pMsg->bytesLeft, n)) {
					DebugMessage("### Request %d: SOCKET->SUBFLOW ###", rr);
				} else if (FindResponse(rr, pMsg->pMsgData + pMsg->bytesOnWire - 8 - pMsg->bytesLeft, n)) {
					DebugMessage("||| Response %d: SOCKET->SUBFLOW |||", rr);
				}
				*/
...
//#endif
			...
			if (PROXY_SETTINGS::bLateBindingLP) {
				...
				if (header[8] == 0 && n > 0) {
					...
					if (k>0) {
						VerboseMessage("Read %d NOP bytes", k);
...
SUBFLOW_MSG * BUFFER_TCP::Allocate(WORD connID, int toPollPos, DWORD seq, WORD len /* include header */) {
	...
	if (fs == BUFFER_FULL) {
		////////////////////////////// 05/19/2014 ////////////////////////////// 
		//This can cause issues when directing specific traffic to certain subflows 
		//conns.EnableSubflowReadNotifications(0);
		////////////////////////////////////////////////////////////////////////
		//InfoMessage("Allocation Failed");
		return NULL;
	} else {
		////////////////////////////// 05/19/2014 ////////////////////////////// 
...
void BUFFER_TCP::RemoveTCPConnection(int pollPos, WORD connID) {
	...
	//the above operations may lead to more buffer space. So try to enable subflow read ntf
