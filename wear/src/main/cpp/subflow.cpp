...
int SUBFLOW::Setup(const char * localIP, const char * remoteIP) {
    ...
    inet_pton(AF_INET, localIP, &clientAddr.sin_addr);

    if (bind(fd[0], (struct sockaddr *)&clientAddr, sizeof(clientAddr)) != 0) {
        LOGD("Cannot bind to it.");
    }
    else{
        LOGD("Binded to %s", localIP);
    }
...
//    SelectPipeFuncUDP_Main = &SUBFLOW::SelectPipe_UDP_SIMPLE;
