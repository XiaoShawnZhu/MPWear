...
//    MyAssert(r == 0, 1762);
    ...
    peers[2].fd = pipes.fd[0];
    peers[2].events = POLLRDNORM;
    return R_SUCC;
}
...
#define EXTRA_BYTES 8
        ...
        if (w > 0) {
            ...
            if (PROXY_SETTINGS::bDumpIO) {
                CONNECTIONS::DumpIO(PIPEBUFFER_2_PIPESOCKET, w, toPollPos, pMHead->connID, pMHead->seq);
            }
             */
        }
...
void BUFFER_PIPE::RemovePipeConnection(int pollPos, WORD connID) {
    LOGD("BUFFER_PIPE::RemovePipeConnection");
    ...
    if (msgLists[pollPos] != NULL) {
        ...
        while (pCur != NULL) {
            ...
            if (pCur->bytesLeft <= 0) {
                ...
                LOGD("*** A: Data Cachesize: %d->%d ***", dataCacheSize, dataCacheSize - (pCur->bytesOnWire));

                dataCacheSize -= pCur->bytesOnWire;
                //anyway the connection will be removed, so no need to decrease bytesInBuffer
            }
...
void CONNECTIONS::RemoveConnection(int pollPos) {
    ...
    //pipeOutput.RemovePipeConnection(pollPos, connID);
...
int BUFFER_PIPE::GetBufferFullStatus(int connID) {
    //quick test
...
void BUFFER_PIPE::ReOrganizeDataCache() {
    //VerboseMessage("BUFFER_TCP::ReOrganizeDataCache");
    unsigned long currT = GetHighResTimestamp();
...
void BUFFER_PIPE::ReOrganizeMsgCache() {
    ...
    //update pCurMsgs
    for (int i=1 + subflows.n; i<=subflows.n + pipes.n; i++) {
        if (pCurMsgs[i] != NULL) {
...
SUBFLOW_MSG * BUFFER_PIPE::Allocate(WORD connID, int toPollPos, DWORD seq, WORD len /* include header */, int pipeNo) {
    LOGD("BUFFER_PIPE::Allocate");
    ...
    pMsg->drop = 0;
    pMsg->bytesLeft = (int)pMsg->bytesOnWire;
    ...
    MyAssert(pMsg->msgType != SUBFLOW_MSG::MSG_CREATE, 1759);
    InsertPipeMsgAndUpdateExpSeq(pMsg, connID, toPollPos, pipeNo);
...
void BUFFER_TCP::InsertSubflowMsgAndUpdateExpSeq(SUBFLOW_MSG * pMsg, WORD connID, int toPollPos, int pipeNo) {
    ...
    //uint64_t start = get_current_microsecond(), end;
    //int oob = 0;//, abandon = 0;
...
//        LOGD("SYN message processed (connID=%d). Exp seq updated to %u", (int)connID, eSeq);
    ...
    LOGD("Pipe message (seq=%u) inserted to TCP buffer (connID=%d). Now expected seq=%u",
         seq, (int)connID, eSeq);
    //oob = GetMsgListBytes(toPollPos);
    //end = get_current_microsecond();
    //oob = GetMsgListBytes(MAX_FDS);
    //printf("Abandon size: %d\n", oob);
    //printf("[%llu][2] connID=%u seq=%u pipe=%d eseq=%u toPollPos=%d outoforder=%d t=%llu\n",
    //	end/1000, connID, seq, pipeNo, eSeq, toPollPos, oob, end-start);
}


void BUFFER_PIPE::InsertPipeMsgAndUpdateExpSeq(SUBFLOW_MSG * pMsg, WORD connID, int toPollPos, int pipeNo) {
    LOGD("BUFFER_PIPE::InsertPipeMsgAndUpdateExpSeq (connID=%d, toPollPos=%d)", (int)connID, toPollPos);
    //uint64_t start = get_current_microsecond(), end;
    //int oob = 0;//, abandon = 0;
...
//        LOGD("*** E: Data Cachesize: %d->%d ***", dataCacheSize, dataCacheSize - (pMsg->bytesOnWire));

        dataCacheSize -= pMsg->bytesOnWire;

        int & bib = conns.connTab[connID].bytesInTCPBuffer;
        bib -= pMsg->bytesOnWire;
        if (bib < 0) bib = 0;
...
//        LOGD("eSeq is %d",eSeq);
    ...
    LOGD("Subflow message (seq=%u) inserted to Pipe buffer (connID=%d). Now expected seq=%u",
         seq, (int)connID, eSeq);
    //oob = GetMsgListBytes(toPollPos);
    //end = get_current_microsecond();
    //oob = GetMsgListBytes(MAX_FDS);
...
void BUFFER_PIPE::RemoveFromAbandonedList(const SUBFLOW_MSG * pMsg) {
    LOGD("BUFFER_TCP::RemoveFromAbandonedList");
...
//    LOGD("CONNECTIONS::TransferFromSubflowsToPipe");
    ...
    while(1){
        ...
        int toFD = conns.peers[2].fd;
        if (r > 0) {
            int w = write(toFD, buf, r);
            sum_w += w;
        }
        if (r == 0){
//            LOGD("---------Exit TransferFromPipesToTCP----------, Read bytes: %d",sum_w);
            return sum_w;
        }
    }
    //blindly fowarding
    ...
    BYTE * header = pipeOutput.headers[fromPollPos];
    SUBFLOW_MSG * & pMsg = pipeOutput.pCurMsgs[fromPollPos];


    //InfoMessage("-------Enter TransferFromPipesToTCP----------");
    while (1) {
        LOGD("Looping in CONNECTIONS::TransferFromSubflowsToPipe");
        if (header[8] == 8) {
            ...
            if (pMsg == NULL) { //allocate space
                ...
                //InfoMessage("Allocate pMsg, connID: %u, seq: %u, pipe: %d", connID, seq, fromPollPos);
#endif
                pMsg = pipeOutput.Allocate(connID, 2/* pipe port*/, seq, len, fromPollPos);
                if (pMsg == NULL) {
            ...
            MyAssert(pMsg->bytesLeft > 0 && pMsg->bytesOnWire >= pMsg->bytesLeft , 1666);

            int n = 0;
            switch (pipeType) {
//#if TRANS == 3		//read from pipe
//			int n = sctp_recvmsg(fromFD, pMsg->pMsgData + pMsg->bytesOnWire - 8 - pMsg->bytesLeft, pMsg->bytesLeft,
//				NULL, NULL, NULL, NULL);
//#elif TRANS == 1
                case SUBFLOW::TCP_PIPE:
                    LOGD("SUBFLOW::TCP_PIPE in CONNECTIONS::TransferFromSublowsToPipe");
                    if (allocate_flag){
                        n = read(fromFD, pMsg->pMsgData + pMsg->bytesOnWire - pMsg->bytesLeft,
                                 pMsg->bytesLeft);
                    }else {
                        n = read(fromFD, pMsg->pMsgData + pMsg->bytesOnWire - pMsg->bytesLeft + 8,
                                 pMsg->bytesLeft - 8);
                        memcpy(pMsg->pMsgData, header, 8);
                        n = n + 8;
                    }
                    break;
//#elif TRANS == 2
                case SUBFLOW::UDP_PIPE:
                    //n = udpRead(fromFD, pMsg->pMsgData + pMsg->bytesOnWire - 8 - pMsg->bytesLeft, pMsg->bytesLeft);
                    break;
//#endif
                default:
                    break;
            ...
            if (n > 0) {
                LOGD("n>0 in CONNECTIONS::TransferFromSubflowsToPipe");
                ...
                /*
                int rr;
                if (FindRequest(rr, pMsg->pMsgData + pMsg->bytesOnWire - 8 - pMsg->bytesLeft, n)) {
                    DebugMessage("### Request %d: SOCKET->PIPE ###", rr);
                } else if (FindResponse(rr, pMsg->pMsgData + pMsg->bytesOnWire - 8 - pMsg->bytesLeft, n)) {
                    DebugMessage("||| Response %d: SOCKET->PIPE |||", rr);
                }
                */
            ...
            if (n == pMsg->bytesLeft) {
                allocate_flag = false;
                //msg fully read
                LOGD("n == pMsg->bytesLeft in CONNECTIONS::TransferFromSublowsToTCP");
                ...
                */
                ...
                if (_pMsg->drop) {
                    pipeOutput.RemoveFromAbandonedList(_pMsg);
                    //InfoMessage("Remove drop, pMsg %p, %u %u",
                    //        _pMsg, _pMsg->connID, _pMsg->seq);
                    //pipeOutput.GetMsgListBytes(MAX_FDS);
                    continue;
                }
                switch (_pMsg->inUseFlag) {
                    case CONN_INFO::CONN_INUSE:
                        //common case: try to transfer immediately  ****important hint*****
                        pipeOutput.TransferFromSubflowsToPipe(2/* pipe port*/);
                        break;
                    ...
                    case CONN_INFO::CONN_EMPTY:
                    {
                        if (_pMsg->msgType == SUBFLOW_MSG::MSG_CREATE) {
                            ...
                            */
                            ...
                            because out-of-order pipe messages - the SYN pipe message is
            ...
            LOGD("Subflow header not fully read yet (%d), continue...", header[8]);
            READ_SUBFLOW_MSG_HEADER:
            LOGD("after head in CONN");
            int nLeft = 8 - header[8];
            LOGD("nLeft in CONN is %d", nLeft);
            int n = 0;
            switch (pipeType) {
//#if TRANS == 3	//read from pipe
//			int n = sctp_recvmsg(fromFD, header + header[8], nLeft, NULL, NULL, NULL, NULL);
//#elif TRANS == 1
                case SUBFLOW::TCP_PIPE:
                    LOGD("before read");
...
//#elif TRANS == 2
                case SUBFLOW::UDP_PIPE:
                    //n = udpRead(fromFD, header + header[8], nLeft);
                    break;
//#endif
                default:
                    break;
            ...
            if (n > 0) {
                ...
                /*
                int rr;
                if (FindRequest(rr, header + header[8], n)) {
                    DebugMessage("### Request %d: SOCKET->PIPE ###", rr);
                } else if (FindResponse(rr, header + header[8], n)) {
                    DebugMessage("||| Response %d: SOCKET->PIPE |||", rr);
                }
                */
            ...
            if (n == nLeft) {
                ...
                //error
                LOGD("Error reading from subflow: %s(%d) Subflow #%d",
...
#endif
    pipeOutput.RemovePipeConnection(2, peersExt[2].connID);
    LOGD("---------Exit TransferFromPipesToTCP----------, Read bytes: %d",nBytesRead);
    return nBytesRead;
}


int CONNECTIONS::TransferFromSubflowsToTCP(int fromPollPos, int fromFD) {
//    LOGD("CONNECTIONS::TransferFromSubflowsToTCP");
    int nBytesRead = 0;
    int pipeType = subflows.GetPipeType(fromFD);
    LOGD("fromFD: %d, pipeType: %d", fromFD, pipeType);
    ...
    //InfoMessage("-------Enter TransferFromPipesToTCP----------");
    while (1) {
        LOGD("Looping in CONNECTIONS::TransferFromSubflowsToTCP");
        if (header[8] == 8) {
            //InfoMessage("[%llu] header read. pCur=%u",
            //		get_current_millisecond(), pMsg);
            if (pMsg == NULL) { //allocate space
...
//#elif TRANS == 1
                case SUBFLOW::TCP_PIPE:
                    LOGD("SUBFLOW::TCP_PIPE in CONNECTIONS::TransferFromSublowsToTCP");
...
//#endif
            ...
            if (n > 0) {
                LOGD("n>0 in CONNECTIONS::TransferFromSubflowsToTCP");
...
//    int toPollPos = (subflows.*subflows.SelectPipeFunc_Main)(connID);
    int toPollPos = 1;
    // InfoMessage("toPollPos is %d", toPollPos);
...
void BUFFER_PIPE::TransferFromSubflowsToPipe(int toPollPos) {
    LOGD("BUFFER_PIPE::TransferFromSubflowsToPipe (toPollPos=%d)", toPollPos);
    ...
    while (pCur != NULL) {
        ...
        switch (pCur->msgType) {
            ...
            {
                LOGD("I am here MSG DATA");
                int x = -pCur->bytesLeft;	//# bytes already written
                int toWrite = pCur->bytesOnWire - x;
                ...
                size += toWrite;
                count += 1;
                LOGD("x is %d, toWrite is %d",x,toWrite);
            ...
            case SUBFLOW_MSG::MSG_CLOSE:
            {
                ...
                msgCacheSize--;
                LOGD("*** B: Data Cachesize: %d->%d ***", dataCacheSize, dataCacheSize - (pCur->bytesOnWire));
                dataCacheSize -= pCur->bytesOnWire ;
        ...
        LOGD("*** C: Data Cachesize: %d->%d ***", dataCacheSize, dataCacheSize - (pCur->bytesOnWire));

        dataCacheSize -= pCur->bytesOnWire;


        int & bib = conns.connTab[conns.peersExt[toPollPos].connID].bytesInTCPBuffer;
        bib -= pCur->bytesOnWire;
        if (bib < 0) bib = 0;
    ...
    while (pCur != NULL) {
        ...
        switch (pCur->msgType) {
            case SUBFLOW_MSG::MSG_DATA:
            {
                ...
                /* ********************* write every pipe msg
                int w = write(toFD, pCur->pMsgData + x, toWrite);
                if (w>0) {
                    nBytesWritten+=w;
                ...
                if (w == toWrite)  {
                    ...
                    //bStop = 0;
                    pCur->bytesLeft -= toWrite;

                    VerboseMessage("Write %d bytes to TCP %s. Full pipe message written (seq=%u)",
                        w, conns.DumpConnection(conns.peersExt[toPollPos].connID), pCur->seq
                    );
                    size += w;
                    //printf("%llu %u %u\n",
                    //        get_current_millisecond(), pCur->connID, pCur->seq);
                    ...
                    //we cannot remove the connection now, because a RST pipe message needs to be sent
                    //protects the closed connection - see the comments at the beginning of TransferFromPipesToTCP()
                    conns.peersExt[toPollPos].establishStatus = POLLFD_EXT::EST_FAIL;
                    conns.TransferFromTCPToPipes(toPollPos, -1); //removing the connection will be handled there
