#ifndef MPWEAR_CONNECTIONS_H
#define MPWEAR_CONNECTIONS_H
...
struct CONNECTIONS {
    ...
    int TransferFromSubflowsToPipe(int fromPollPos, int fromFD);
};
...
struct BUFFER_PIPE{
    ...
    void Setup();
    void NewTCPConnection(int pollPos);
    void RemovePipeConnection(int pollPos, WORD connID);


    void RemoveFromAbandonedList(const SUBFLOW_MSG * pMsg);
    void InsertPipeMsgAndUpdateExpSeq(SUBFLOW_MSG * pMsg, WORD connID, int toPollPos, int pipeNo);
    ...
    void TransferFromSubflowsToPipe(int toPollPos);
